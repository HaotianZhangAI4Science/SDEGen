...
def run_a_train_epoch(epoch, model, data_loader, optimizer, aux_weight=0.001, device='cpu'):
	...
	for batch_id, batch_data in enumerate(data_loader):
		...
		dist_pl_real, dist_pl_pred, pl_batch, dist_ll_real, dist_ll_pred, ll_edge_batch = model(bgl, bgp)
		...
		#del bgl, bgp, dist_pl_real, dist_pl_pred, pl_batch, dist_ll_real, dist_ll_pred, ll_edge_batch
		th.cuda.empty_cache()
...
def run_an_eval_epoch(model, data_loader, pred=False, aux_weight=0.001, device='cpu'):
	model.eval()
	total_loss = 0
	mdn_loss = 0
	atom_loss = 0
	...
	with th.no_grad():
		for batch_id, batch_data in enumerate(data_loader):
			...
			#	prob = scatter_add(prob, batch, dim=0, dim_size=bgl.batch_size)
			#	probs.append(prob)
			#else:
